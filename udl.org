...
𝐱 = [0.03, 0.19, 0.34, 0.46, 0.78, 0.81, 1.08, 1.18, 1.39, 1.60, 1.65, 1.90]
𝐲 = [0.67, 0.85, 1.05, 1.0, 1.40, 1.5, 1.3, 1.54, 1.55, 1.68, 1.73, 1.6]

f(𝐱, 𝛟) = muladd.(𝐱, 𝛟[2], 𝛟[1])
L(𝛟) = sum(abs2, f(𝐱, 𝛟) .- 𝐲)
#+end_src
...
#+begin_src jupyter-julia :session jl :async yes
function compute_and_plot_loss(;𝛟)
    𝐱ₚ = 0:2
    𝐲ₚ = f(𝐱ₚ, 𝛟)

    println("L(ϕ₀=$(𝛟[1]), ϕ₁=$(𝛟[2])) = $(L(𝛟))")
    plot(𝐱ₚ, 𝐲ₚ,
        label=L"f_{\boldsymbol{\phi}}(x)",
        xlabel=L"x",
        ylabel=L"y"
    )
    scatter!(𝐱, 𝐲, label="Data")
end

compute_and_plot_loss(𝛟=[0.4, 0.2])
#+end_src

:RESULTS:
: L(ϕ₀=0.4, ϕ₁=0.2) = 7.067864
[[file:./.ob-jupyter/5cb02ae2a5cdbf10ecd225020150beb99ac64b38.svg]]
:END:

#+begin_src jupyter-julia :session jl :async yes
compute_and_plot_loss(𝛟=[1.6, -0.8])
#+end_src

:RESULTS:
: L(ϕ₀=1.6, ϕ₁=-0.8) = 10.279524

[[file:./.ob-jupyter/07fac69936ba4574f909e5b121a81ce74e30cccb.svg]]
:END:

#+begin_src jupyter-julia :session jl :async yes
compute_and_plot_loss(𝛟=[0.8, 0.5])
#+END_src

:RESULTS:
: L(ϕ₀=0.8, ϕ₁=0.5) = 0.22852499999999987
[[file:./.ob-jupyter/eecc6c943abebcc3ca18f795284140a3acd5ba49.svg]]
:END:

#+begin_src jupyter-julia :session jl :async yes
heatmap(0.75:0.01:0.9, 0.45:0.01:0.6, (ϕ₀, ϕ₁) -> L([ϕ₀, ϕ₁]),
    clims=(0.2, 0.25),
...
-----
*2.1.* /To walk “downhill” on the loss function (equation 2.5), we measure its gradient with respect to the parameters \( \phi_0 \) and \( \phi_1 \). Calculate expressions for the slopes \( \partial L / \partial \phi_0 \) and \( \partial L / \partial \phi_1 \)./
-----
...
In conclusion, a nonlinear activation function is necessary to “break” the function’s linearity.
...
-----
*3.3* /Derive expressions for the positions of the “joints” in function in figure 3.3j in terms of the ten parameters \( \boldsymbol{\phi} \) and the input \( x \). Derive expressions for the slopes of the four linear regions./
-----

The “joints” take place where the linear functions pass through 0, which is where the ReLU “cut” happens, resulting in the four distinct slopes:
...
plot(
    plot(f₃, 0, 2, title=L"\theta_{30} + \theta_{31}x"),
    plot(x -> a(f(x)), 0, 2, title=L"h_3 = \mathrm{a}[\theta_{30} + \theta_{31}x]"),
    plot(x -> 0.7 * a(f₃(x)), 0, 2, title=L"\phi_3 h_3"),
    plot(x -> -1.2 * a(f₁(x)) + 1.1 * a(f₂(x)) + 0.7 * a(f₃(x)), 0, 2, title=L"\phi_1 h_1 + \phi_2 h_2 + \phi_3 h_3"),
    ylims=(-1, 1),
    lw=2,
    legend=false,
)
...
-----
*3.7* /Consider fitting the model in equation 3.1 using a least squares loss function. Does this loss function have a unique minimum? i.e., is there a single “best” set of parameters?/
-----
...
function plot_activation(a, plot_title)
    f(x) = ϕ[1] + ϕ[2] * a(l₁(x)) + ϕ[3] * a(l₂(x)) + ϕ[4] * a(l₃(x))

    fig_data = [
        (x -> l₁(x), L"\theta_{10} + \theta_{11} x"),
        (x -> l₂(x), L"\theta_{20} + \theta_{21} x"),
        (x -> l₃(x), L"\theta_{30} + \theta_{31} x"),
        (x -> a(l₁(x)), L"h_1 = \mathrm{a}[\theta_{10} + \theta_{11} x]"),
        (x -> a(l₂(x)), L"h_2 = \mathrm{a}[\theta_{20} + \theta_{21} x]"),
        (x -> a(l₃(x)), L"h_3 = \mathrm{a}[\theta_{30} + \theta_{31} x]"),
        (x -> ϕ[1] * a(l₁(x)), L"\phi_1 h_1"),
        (x -> ϕ[2] * a(l₂(x)), L"\phi_2 h_2"),
        (x -> ϕ[3] * a(l₃(x)), L"\phi_3 h_3"),
...
/heaviside:/ Piecewise “constant” functions with 4 constant regions.
...
/tanh:/ Piecewise hyperbolic functions with 4 regions. Perhaps a “smoothed-out” version of the /heaviside/ function, where the transition is a hyperbola instead of a direct jump.
...
/rect:/ Piecewise “constant” functions with 7 constant regions.
...
y, Z, H, H_w = shallow_nn(x, ReLU, ϕ, Θ)
...
least_squares_loss(y, ŷ) = sum(abs2, y .- ŷ)
...
y_predict, Z, H, H_w = shallow_nn(x_train, ReLU, ϕ, Θ)
...
function shallow_nn_2D(X, a, ϕ, Θ)
    # X : (D x N)
    @views b_in = Θ[:, 1]       # (K)
    @views W_in = Θ[:, 2:end]   # (K x D)
    @views b_out = ϕ[:, 1]      # (O)
    @views W_out = ϕ[:, 2:end]  # (O x K)

    Z = (W_in * X) .+ b_in                           # (K x N)
    H = a.(Z)                                        # (K x N)
    H_w = reshape(W_out', size(W_out')..., 1) .*     # (K x O x N)
          reshape(H, size(H, 1), 1, size(H, 2))
    Y = dropdims(sum(H_w, dims=1), dims=1) .+ b_out  # (O x N)

    return Y, Z, H, H_w
end

function plot_nn_2D(x, Y, Z, H, H_w)
    fig_data = [
        (x, x, Z[1, :], L"\theta_{10} + \theta_{11} x_1 + \theta_{12} x_2"),
        (x, x, Z[2, :], L"\theta_{20} + \theta_{21} x_1 + \theta_{22} x_2"),
        (x, x, Z[3, :], L"\theta_{30} + \theta_{31} x_1 + \theta_{32} x_2"),
        (x, x, H[1, :],
         L"h_1 = \mathrm{a}[\theta_{10} + \theta_{11} x_1 + \theta_{12} x_2]"),
        (x, x, H[2, :],
         L"h_2 = \mathrm{a}[\theta_{20} + \theta_{21} x_1 + \theta_{22} x_2]"),
        (x, x, H[3, :],
         L"h_3 = \mathrm{a}[\theta_{30} + \theta_{31} x_1 + \theta_{32} x_2]"),
        (x, x, H_w[1, 1, :], L"\phi_{11} h_1"),
        (x, x, H_w[2, 1, :], L"\phi_{12} h_2"),
        (x, x, H_w[3, 1, :], L"\phi_{13} h_3"),
        (x, x, H_w[1, 2, :], L"\phi_{21} h_1"),
        (x, x, H_w[2, 2, :], L"\phi_{22} h_2"),
        (x, x, H_w[3, 2, :], L"\phi_{23} h_3"),
        (x, x, Y[1, :],
         L"\phi_{10} + \phi_{11} h_1 + \phi_{12} h_2 + \phi_{13} h_3"),
        (x, x, Y[2, :],
         L"\phi_{20} + \phi_{21} h_1 + \phi_{22} h_2 + \phi_{23} h_3")
    ]

    subplots = map(fig_data) do (x, y, z, title)
        N = Int(√length(z))
        Zᵣ = reshape(z, (N, N))
        p = heatmap(x, y, Zᵣ,
            title=title,
        ...
        contour!(p, x, y, Zᵣ, cmap=:white, opacity=0.25)
    end

    plot(subplots..., layout=(5, 3), size=(1000, 1200))
end
...
ϕ = [ 0.0 -2.0  2.0  1.5
     -2.0 -1.0 -2.0  0.8]
x = 0:0.1:10
X = stack(vec(collect(Iterators.product(x, x))))

Y, Z, H, H_w = shallow_nn_2D(X, ReLU, ϕ, Θ)
plot_nn_2D(x, Y, Z, H, H_w)
#+end_src

#+RESULTS:
[[file:./.ob-jupyter/6eb0a93a049d649b85e7ead4d3ea114bfb4543f3.svg]]
* Notebook 3.3 --- Shallow Network Regions

#+begin_src jupyter-julia :session jl :async yes :results output
number_regions(Dᵢ, D) = sum(binomial(BigInt(D), j) for j in 0:Dᵢ)

@show number_regions(2, 3)
@show number_regions(10, 50)
@show number_regions(10, 8)
#+end_src

#+RESULTS:
: number_regions(2, 3) = 7
: number_regions(10, 50) = 13432735556
: number_regions(10, 8) = 256
...
units = 0:1000
dims = [1, 5, 10, 50, 100]
regions = [number_regions(Dᵢ, D) for D in units, Dᵢ in dims]

idxs = 5:-1:1
plot(units, regions[:, idxs],
    yscale=:log10,
    ylims=(1e0, 1e150),
    label=[L"D_i=%$d" for d in dims[idxs]'],
    xlabel=L"\text{Number of hidden units, } D",
    ylabel=L"\text{Number of regions, } N"
)
#+end_src

#+RESULTS:
[[file:./.ob-jupyter/06516a940635ef6c98473f2462c724d44810bd3b.svg]]


#+begin_src jupyter-julia :session jl :async yes :results output
number_parameters(Dᵢ, D) = D * (Dᵢ + 2) + 1

@show number_parameters(2, 3)
@show number_parameters(10, 50)
@show number_parameters(10, 8)
#+end_src

#+RESULTS:
: number_parameters(2, 3) = 13
: number_parameters(10, 50) = 601
: number_parameters(10, 8) = 97
...
units = 0:200
dims = [1, 5, 10, 50, 100]
regions = [number_regions(Dᵢ, D * 500 / Dᵢ) for D in units, Dᵢ in dims]
parameters = [number_parameters(Dᵢ, D * 500 / Dᵢ) for D in units, Dᵢ in dims]

idxs = 5:-1:1
plot(parameters[:, idxs], regions[:, idxs],
    yscale=:log10,
    xlims=(0, 100_000),
    ylims=(1e0, 1e150),
    label=[L"D_i=%$d" for d in dims[idxs]'],
    xlabel=L"\text{Number of parameters, } D",
    ylabel=L"\text{Number of regions, } N"
)
#+end_src

#+RESULTS:
[[file:./.ob-jupyter/5eb42615cebd890d3390ad44125516089fc5519c.svg]]

* Problems 3.14--3.17 (Shallow Neural Networks)

-----
*3.14* /Write out the equations that defnie the network in figure 3.11. There should be three equations to compute the three hidden units from the inputs and two equations to compute the outputs from the hidden units./
-----
...
-----
*3.15* /What is the maximum possible number of 3D linear regions that can be created by the network in figure 3.11?/
-----

\begin{align*}
\text{N. regions} &= \sum_{j=0}^{D_i} \binom{D}{j} = \sum_{j=0}^{3} \binom{3}{j} \\
...
-----
*3.16* /Write out the equations for a network with two inputs, four hidden units, and three outputs. Draw this model in the style of figure 3.11./
-----
...
h_2 = \mathrm{a}[\theta_{20} + \theta_{21} x_1 + \theta_{22} x_2] \\
h_3 = \mathrm{a}[\theta_{30} + \theta_{31} x_1 + \theta_{32} x_2] \\
h_4 = \mathrm{a}[\theta_{40} + \theta_{41} x_1 + \theta_{42} x_2] \\
\\
...
\begin{center}
\begin{tikzpicture}[
    node distance=0.25cm and 2cm,
    layer/.style={circle, draw, thick, minimum size=0.75cm, inner sep=0pt},
    arrow/.style={->, >={Stealth[width=1.25mm,length=2mm]}, thick}
]
    \node[layer] (x1) {$x_1$};
    \node[layer, below=of x1] (x2) {$x_2$};
    \node[layer, right=of x1] (h2) {$h_2$};
    \node[layer, above=of h2] (h1) {$h_1$};
    \node[layer, below=of h2] (h3) {$h_3$};
    \node[layer, below=of h3] (h4) {$h_4$};
    \node[layer, right=of $(h2.center)!0.5!(h3.center)$] (y2) {$y_2$};
    \node[layer, above=of y2] (y1) {$y_1$};
    \node[layer, below=of y2] (y3) {$y_3$};
    \foreach \i in {1,2}
...
-----
*3.17* /Equations 3.11 and 3.12 define a general neural network with \( D_i \) inputs, one hidden layer containing \( D \) hidden units, and \( D_o \) outputs. Find an expression for the number of parameters in the model in terms of \( D_i \), \( D \), and \( D_o \)./
-----

\begin{align*}
\text{N. parameters} = D(D_i + 1) + D_o(D + 1)
\end{align*}
...
function plot_activation(a, plot_title)
    ...
    subplots = map(fig_data) do (f, title)
        plot(f, x, title=title, titlefontsize=11)
    end
...
#+RESULTS:
[[file:./.ob-jupyter/28ff61b5eb153c8081927b18908c5f93b6a46610.svg]]
...
#+RESULTS:
[[file:./.ob-jupyter/c551c961bd18f62abe1a29f7c36375171e0af634.svg]]
...
#+RESULTS:
[[file:./.ob-jupyter/01279e2b553f79add76daa7f75c480003aa62357.svg]]

#+begin_src jupyter-julia :session jl :async yes
lin(z; a=0.5, b=-0.4) = a + b * z
plot_activation(lin, "lin")
#+end_src

#+RESULTS:
[[file:./.ob-jupyter/543e501dbbdd09fe8297ee9987017217a2f5fe99.svg]]

* Problem 3.18 (Shallow Neural Networks)
...
\begin{align*}
\sum_{j=0}^{2} \binom{3}{j} &= \binom{3}{0} + \binom{3}{1} + \binom{3}{2} = 1 + 3 + 3 = 7 & (D = 3) \\
\sum_{j=0}^{2} \binom{5}{j} &= \binom{5}{0} + \binom{5}{1} + \binom{5}{2} = 1 + 5 + 10 = 16 & (D = 5)
\end{align*}
* Problem 4.1 (Deep Neural Networks)

-----
*4.1* Consider composing the two neural networks in figure 4.8. Draw a plot of the relationship between the input \( x \) and output \( y' \) for \( x \in [-1, 1] \).
-----
...
x = [-1, -0.8, -0.6, -0.35, -0.25, -0.2, 0, 0.2, 0.3, 0.4, 0.75, 0.85, 1]
y = [-0.8, -0.7, 0, 0, -0.7, -0.8, 0.6, -0.8, -0.7, 0, 0, -0.7, -0.8]

plot(x, y, xlims=(-1, 1), ylims=(-1, 1), xlabel=L"x", ylabel=L"y'", legend=false)
#+end_src

#+RESULTS:
[[file:./.ob-jupyter/0b47ffc3d20a07678a433208cc734956fcbdd1c7.svg]]
